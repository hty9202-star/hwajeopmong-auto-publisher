...
// 도입부 스타일 변형
const INTRO_STYLES = [
  '흔히 겪는 상황이나 고민을 서술형으로 시작하세요. 예시 문구를 베끼지 말고 이번 글의 질환·부위에 맞는 새로운 장면을 직접 만드세요.',
  '핵심 정의문으로 시작하세요 (형식 예: "~은(는) ~에 의해 발생하는 피부 질환입니다." — 내용은 이번 질환에 맞게).',
  '흥미로운 의학적 사실이나 통계적 경향으로 시작하세요.',
  '계절이나 환경 변화에 연결하여 시작하세요.',
  '증상 묘사로 시작하세요. 이번 글의 부위·양상에 맞는 구체적 증상 장면으로 여세요.',
];
...
const INTRO_CASES = [
  '바쁜 일상 속에서 증상을 방치하게 되는 상황으로 여세요. 단, 특정 실제 환자가 아니라 "이런 분들이 많다"는 일반화된 톤으로.',
  '계절·시점 계기로 여세요(환절기, 장마철, 냉난방, 휴가 전 등 이번 질환과 잘 맞는 시점 하나를 고르세요).',
  '흔한 오해나 시행착오로 여세요(다른 질환으로 착각, 자가 처치 반복, 검색으로 자가진단 등).',
  '옷차림·노출 상황과 연결해 여세요(반팔·짧은 옷·수영장·목폴라 등 부위가 드러나거나 가려지는 상황).',
  '아침 세면·샤워·화장 등 루틴 중 증상을 자각하는 순간으로 여세요.',
  '주변 계기로 여세요(지인·가족의 권유, 사진 속 피부를 보고 등).',
  '독자에게 던지는 담백한 질문으로 여세요(억지 "있으신가요?"는 금지).',
  '헷갈리기 쉬운 다른 질환과 구분하는 지점으로 여세요.',
  '치료를 미루다 악화된 흐름으로 여세요(초기엔 대수롭지 않게 여기다가).',
  '재발의 답답함에 공감하며 여세요(나아졌다 다시 올라오는 반복). 단, 불안·공포를 과하게 조장하지 말 것.',
  '연령·생활 단계와 연결해 여세요(사춘기, 사회초년생, 출산 후, 갱년기 등 이번 질환과 맞는 대상 하나).',
  '날씨·땀·마스크 등 물리적 자극 상황과 연결해 여세요.',
  '피부과 시술·약을 시도했지만 만족스럽지 않았던 경험으로 여세요(타 병원·시술 비방은 금지, 담담한 서술).',
  '가려움·따가움 등 감각 불편이 일상을 방해하는 장면으로 여세요.',
  '사람을 만나거나 사진 찍는 자리에서 신경 쓰이는 심리로 여세요. 단, 불안·공포를 과하게 조장하지 말 것.',
  '증상의 위치·모양을 담담히 관찰하듯 묘사하며 여세요.',
];
...
function loadReferenceContent(topic) {
  ...
  try {
    ...
    if (topic && topic.name) {
      ...
      // "핵심" 원고는 그 질환의 기준 문서이므로 항상 우선 포함하고, 나머지에서 랜덤 보충
      const coreFiles = matched.filter(function(f) { return f.includes('핵심'); });
      const otherFiles = matched.filter(function(f) { return !f.includes('핵심'); });
      const shuffledOthers = [...otherFiles].sort(() => Math.random() - 0.5);
      const orderedMatched = [...coreFiles, ...shuffledOthers];
      const matchCount = Math.min(Math.max(2, coreFiles.length), orderedMatched.length, sampleCount);
      selected.push(...orderedMatched.slice(0, matchCount));
    }
...
const TREATMENT_MAP = {
  ...
  'keratosis-pilaris': { name: '화안케어', desc: '모공각화증은 화접몽한의원의 화안케어로 치료한다. 화안케어는 파우더필링으로 한약 추출물을 표피층에 침투시켜 두껍게 쌓인 각질이 제거되도록 하는 치료 방식으로, 화학 필링이 아니라 피부 자체의 재생력을 강화하며 모공각화증과 함께 착색된 피부 톤까지 보정한다. ※미세약초침이 아니라 파우더필링임 — "미세약초침"이라는 표현을 쓰지 말 것. 모공각화증은 타입에 따라 치료 강도가 달라진다: (1) 매몰모형 — 각질과 모공 사이에 빠져나오지 못한 털이 피지·세균과 엉겨 검붉거나 갈색 착색을 만드는, 가장 흔하고 잘 치료되는 타입(여성 대부분)이며 각질 교체 주기를 정상화하면 빠르게 호전된다. (2) 호르몬민감형 — 모근이 호르몬에 민감해 모공 하나에서 털이 두 개 이상 나는 유형으로, 매몰모보다 붉고 깊으며 유전 영향이 커 일반 필링만으로는 한계가 있어 강도 높은 화안케어가 필요하다. (3) 홍반형 — 육안상 가장 심한 유형으로 90% 이상이 남성이고 군집 형태(사이사이 정상피부)를 띠어 강력한 화안케어가 필요하다. 이태리타올로 미는 목욕, 손톱으로 뜯기, 면도가 대표적 악화 요인이다.' },
  'acne': { name: '화안케어 + 한약치료 + 여드름 압출', desc: '여드름은 화안케어(미세침)·한약치료·여드름 압출을 병행하는 치료다. 외부 치료인 화안케어는 한약 추출물을 미세침으로 피부에 침투시켜 각질탈락주기를 정상화하고 유수분 밸런스를 조절하며, 내과적으로는 한약치료(복용)로 반복되는 염증 경향을 체질적으로 개선한다. 또한 피지 덩어리가 커지면 피부가 스스로 정화하기 어렵고 추후 큰 염증을 일으켜 자국을 남기므로, 치료가 능숙한 의료진이 여드름을 압출하고 후처치하는 과정이 필요하다. 이때 실즉사 허즉보(實則瀉 虛則補) 원칙으로 접근해 여드름 치료 기간을 단축하고 흉터 발생 가능성을 낮춘다. 단, 압출과 진정 치료만으로는 한계가 있을 수 있어 안팎 병행이 중요하다. ※한약을 "도포"하는 것이 아니라 복용하는 한약치료임.' },
  '등여드름': { name: '화안케어', desc: '등여드름은 화접몽한의원의 화안케어로 치료한다. 화안케어는 한약 추출물을 미세침을 이용해 피부 속에 침투시켜 각질 제거, 피지선 위축, 피부 장벽 기능 회복, 미백, 여드름 자국 개선을 돕는 치료법이다. ※등여드름은 화안케어만 언급하고, 한약 복용 등 다른 치료법은 언급하지 말 것.' },
  'acne-scars': { name: '리셀테라피(복합 흉터치료)', desc: '화접몽의 복합 여드름흉터 치료 프로그램인 리셀테라피로 진행한다. 치료의 첫 단계는 패이고 울퉁불퉁해진 피부 표면을 정돈하는 과정으로, 약물을 이용해 흉터의 경계면을 세밀하게 자극하고 피부가 스스로 회복 반응을 일으키도록 돕는다. 이후 화안케어, 고밀도 AMTS, 화안케어 순으로 재생 과정이 이어지며 흉터의 깊이와 피부 결·색소침착을 함께 개선한다. ※"한약도포"라는 용어는 과정 설명에 쓰지 말 것(언급하지 않아도 됨). "한약 성분을 도포한다"는 식으로도 설명하지 말고, 위처럼 "표면 정돈 + 약물로 경계면 자극 + 스스로 회복 유도" 방식으로만 서술한다. 리셀테라피는 연구논문으로 발표된 치료법이다.' },
  ...
  'atopic-dermatitis': { name: '발효약초테라피 + 한약', desc: '아토피는 ① 발효약초테라피(피부질환 재생 중심 — 발효약초팩으로 아프고 갈라지고 탈락되는 피부를 빠르게 진정)와 ② 환자의 다양한 증상에 맞춘 한약 처방을 병행해 면역 균형을 회복하고 피부 장벽을 강화하는 치료법.' },
  // 건선: 탈스(脫스테로이드)케어 — "면역 억제가 아닌 면역 회복" 관점의 3단계 치료 (건선 핵심1·핵심2 원고 기준, 2026-07 갱신)
  'psoriasis': { name: '탈스테로이드 케어 (3단계 맞춤 치료)', desc: '건선은 피부 건조 문제가 아니라, 전신 면역체계의 일부가 과도하게 활성화되며 피부 세포 재생 주기가 정상보다 약 8~10배 빨라져 미성숙 각질이 두껍게 쌓이는 만성 전신 면역 질환이다. 연고를 바르면 잠시 가라앉고 끊으면 더 심하게 튀어오르는 반동(리바운드)의 악순환이 반복되기 쉽다. 화접몽한의원은 면역을 누르는 치료가 아니라 스스로 회복하는 탈(脫)스테로이드 케어를 3단계로 진행한다. (1) 맞춤형 내치 한약 처방(면역 균형·트리거 제거) — 맥진·설진·복진으로 환자 고유의 체질적 약점과 건선을 일으킨 몸속 열독, 즉 "건선 트리거"를 찾아낸다. 트리거는 소화기 문제(잘 체하고 독소가 쌓이는 체질), 극심한 스트레스(속열), 잦은 감기·편도염 후 열독, 땀 배출 기능 이상, 기허·혈허(만성 피로·출산·수술 후 소모), 과로·수면부족, 스테로이드·면역억제제 오남용에 의한 면역 반동의 7가지 축으로 추적하며, 과열된 면역을 안정시키고 독소 배출을 돕는 1:1 개인별 한약을 처방한다. (2) 탈스테로이드 안심 케어(리바운드 제어) — 의존해온 스테로이드·면역억제제를 줄이거나 끊을 때 나타나는 피부 뒤집어짐·진물·각질 폭발 등 반동 현상을 안전하게 통과하도록 외치 완화 프로그램과 밀착 생활 관리를 병행한다. (3) 피부 장벽 재생 치료(각질 주기 정상화) — 천연 한약재 성분을 추출한 약침 치료와 특정 파장의 광선 치료를 병행해 붉고 두꺼워진 병변의 염증을 직접 가라앉히고, 인위적으로 각질을 벗겨내는 것이 아니라 빨라진 재생 주기를 정상화해 매끄러운 새살이 돋도록 돕는다. 증상 감별 시 은백색 인설, 오스피츠 사인(각질을 떼면 점상 출혈), 쾨브너 현상(자극 부위로 번짐), 손발톱 함몰(네일 피팅) 등을 참고할 수 있다. ※발효약초테라피 등 다른 질환의 치료법을 끌어와 섞지 말 것. "완치", "반드시 낫는다" 같은 단정 표현은 쓰지 말 것.' },
  'dyshidrosis': { name: '발효약초테라피 + 한약', desc: '습진·한포진은 ① 발효약초테라피와 ② 한약을 병행해 치료한다. 발효약초테라피는 피부질환 재생을 중심으로, 아프고 갈라지고 탈락되는 피부를 발효약초팩으로 빠르게 진정시키며 손상된 피부 조직의 해면화·태선화 회복을 돕는다. 한약은 환자의 다양한 증상에 맞춰 처방한다.' },
  'diet': { name: '감비환 한방 다이어트', desc: '화접몽한의원의 체질 맞춤 한약 처방인 감비환과 식이 관리를 병행하는 한방 체중 관리 프로그램.' },
  'seborrheic-dermatitis': { name: '한약치료 + 화안케어', desc: '지루성피부염은 ① 한약 처방(복용)으로 자율신경을 정상화하고 호르몬·체내 열을 조절하며, ② 화안케어(미세침을 통한 약초 성분 전달)로 피부 염증을 완화하는 치료다. 아울러 보습 관리와 유산균 섭취 등 생활 관리도 치료 못지않게 중요하게 함께 안내한다.' },
  // 피부아밀로이드증: 병변 깊이·상태 진단 후 거우침 → 화안케어 → 내치 한약의 단계별 1:1 맞춤 프로세스. 이 3단계 외 다른 질환 치료법을 끌어오지 말 것.
  'cutaneous-amyloidosis': { name: '거우침 + 화안케어 + 내치 한약 (단계별 맞춤)', desc: '피부아밀로이드증은 환자마다 다른 병변의 깊이와 상태를 진단한 뒤 단계별 1:1 맞춤 프로세스로 치료한다. (1) 거우침(巨宇鍼) — 오래되어 두껍고 딱딱하게 고착화된 아밀로이드 침착 부위와 두꺼워진 변성 조직을 물리적으로 자극해 탈락을 유도하고, 신선한 정상 피부 세포가 재생될 수 있는 환경을 조성한다. (2) 화안케어 — 두꺼운 돌기와 각질층이 정리되면 피부 재생 주기에 맞춰 진행하며, 미세침 성분으로 진피와 표피의 동시 재생을 촉진해 거칠어진 피부결을 부드럽게 하고 깊게 정체된 갈색 색소 침착을 점진적으로 배출시킨다. (3) 내치(內治) 한약 — 재발의 가장 큰 원인인 가려움증을 다스리기 위해 체내 열독을 내리고 면역 균형을 맞추는 맞춤 한약을 병행하여 긁는 습관 자체를 교정한다. ※거우침·화안케어·내치 한약 3단계 구성이며, 다른 질환의 치료법을 끌어와 섞지 말 것.' },
};
...
async function stageStrategy(apiKey, topic, contentType, research, existingTitles = []) {
  ...
  // ─── 차별화 축(facet) 로테이션 ───
  // 같은 질환+유형(comboId)의 첫 글은 대표글(대표 키워드), 2번째 글부터는 서로 다른
  // 롱테일 축을 강제 배정해 카니벌라이제이션(같은 키워드 자기잠식)을 방지한다.
  const FACET_POOL = [
    { label: '부위별', hint: '특정 부위(등·가슴·얼굴·팔·다리·두피 등) 하나에 집중해 "부위+질환" 롱테일을 노리세요.' },
    { label: '연령·대상별', hint: '특정 연령·대상(10대·20~30대·성인·직장인 등)에 맞춘 관점으로 좁히세요.' },
    { label: '상황·계기별', hint: '특정 상황·시점(환절기·여름·겨울·출산 후·스트레스·과로 등)과 결합하세요.' },
    { label: '증상·단계별', hint: '특정 증상·단계(활동성/비활동성, 초기/만성, 색소침착·번짐 등)에 집중하세요.' },
    { label: '비교·감별', hint: '치료 방식의 차이나 유사 질환과의 감별 관점으로 좁히세요.' },
    { label: '재발·예방·관리', hint: '재발 원인과 예방·관리에 집중하는 관점으로 좁히세요.' },
  ];
  const assignedFacet = existingTitles.length > 0
    ? FACET_POOL[(existingTitles.length - 1) % FACET_POOL.length]
    : null;
...
- 괄호() 사용 금지
${contentType.id === 'korean-medicine-treatment' ? `- ★브랜드 자연 포함 — 이 유형만 적용★: 제목에 "강남 화접몽한의원"을 ★문장 안에 자연스럽게 녹여서★ 포함하세요. 덧붙이는 게 아니라 문장의 구성 요소가 되어야 합니다.
  * (O) "모낭염 재발을 멈추는 강남 화접몽한의원 치료법"
  * (O) "강남 화접몽한의원이 아토피 상열감을 다루는 방식"
  * (X) "강남에서 …" — 지역을 문두 부사어로 쓰지 말고 "강남 화접몽한의원"을 하나의 단위로 붙여 쓰세요.
  * (X) "… | 강남 화접몽한의원" — 제목 끝에 기계적으로 덧붙이지 마세요.
  * 표기는 반드시 "화접몽한의원"(붙여쓰기). "화접몽 한의원"처럼 띄어 쓰지 마세요.
  * 브랜드를 넣어도 제목이 늘어지지 않게, 요소는 2~3개로 간결하게 유지하세요.` : `- ★브랜드·지역 제외 — 반드시 준수★: 제목에 브랜드명("${BRAND.name}", "화접몽")이나 지역(강남·강남역)을 절대 넣지 마세요. 브랜드·지역은 검색결과용 SEO 타이틀 뒤쪽에 자동으로 붙습니다. 글 제목은 ★환자가 검색창에 실제로 치는 질문·증상 문장★ 그 자체로만 작성하세요.`}
- 일반 예시 (구조를 다양하게 — 매번 다른 패턴 사용):
  * 질문형: "편평사마귀는 옮나요"
  * 증상형: "정강이에 오돌토돌 갈색 돌기가 올라온다면"
...
- ★검색어 일치 — 매우 중요★: 제목은 환자가 실제로 검색창에 입력하는 표현에 최대한 가깝게 쓰세요. 의학 전문용어보다 환자가 쓰는 말("오돌토돌", "옮나요", "자꾸 올라와요", "없어지나요")을 우선합니다. 또한 한 제목에 여러 요소를 욱여넣어 늘어지지 않도록 요소는 2~3개로 간결하게 유지하세요.
- ★이번 제목의 관점(이 관점을 우선 적용)★: ${(() => {
  var t = topic.name;
  var questionAngle = '질문형 — 환자가 검색창에 그대로 칠 법한 질문으로 쓰세요(예: "' + t + ' 옮나요", "…해도 되나요", "왜 자꾸 …", "…어떻게 해야 할까", "…없어지나요"). 억지 질문("있으신가요?")은 금지.';
  var symptomAngle = '증상 서술형 — 환자가 실제로 느끼는 증상·상태를 그대로 묘사해 검색어와 일치시키세요(예: "정강이에 오돌토돌 갈색 돌기가 올라온다면", "긁을수록 진해지는 색소침착"). 질문형이 아니어도 됩니다.';
  var angles = [
  ...
  // 질문형 45% (환자 실제 검색 문장에 가장 가까움)
  if (Math.random() < 0.45) return questionAngle;
  // 증상 서술형 (나머지 55% 중 약 35% → 전체의 약 19%)
  if (Math.random() < 0.35) return symptomAngle;
...
- ★부위·대상 반복 금지★: 위 기존 제목들에 이미 등장한 부위·대상(예: 엉덩이, 두피, 등)을 이번 제목의 주력 소재로 다시 쓰지 마세요. 서브토픽 목록에서 아직 다루지 않은 것을 고르세요.

★이번 글의 차별화 축(반드시 적용) — ${assignedFacet.label}★
${assignedFacet.hint}
- 위 기존 글들이 "${topic.name}" 대표 키워드를 이미 다뤘습니다. 이 글은 같은 대표 키워드를 반복하지 말고, 위 축으로 좁힌 "롱테일 검색어"를 주력으로 삼으세요.
- 이 축이 제목뿐 아니라 H2 소제목·본문 키워드 전체를 이끌게 하세요. (제목만 바꾸고 본문이 개요면 실패입니다.)
- 이 축이 "${topic.name}"에 잘 맞지 않으면, 기존 글과 겹치지 않는 가장 가까운 다른 축으로 바꿔도 됩니다.
- 단, 화접몽 치료법(거우침·화안케어 등) 설명은 축과 무관하게 그대로 포함하세요.
` : ''}
[소제목(H2) 작성 규칙 - 매우 중요]
...
※ 위 소재의 예시 표현을 그대로 베끼지 말고, 이번 글의 질환·부위·상황에 맞는 새로운 도입 장면을 직접 창작하세요. 특히 "야근", "거울을 볼 때마다", "화장으로 가리다" 등 반복적으로 쓰여 상투적으로 느껴지는 도입 표현은 사용하지 마세요.
...
14. 언어: 한국어로만 작성. 의학 용어 영문 병기만 허용. ★한 단어 안에서 한글과 영문을 섞은 반쪽 표기 절대 금지★ — 외국어 의학용어는 「완전한 한글 음역」 또는 「완전한 영문 원어」 중 하나로만 쓰세요. 예: "몰uscum"(X) → "molluscum" 또는 "몰러스컴"(O), "HPV바이러스"처럼 붙여쓰지 말고 "HPV 바이러스"(O). 영문 철자 일부만 한글로 바뀐 깨진 단어가 나오지 않도록 각별히 주의하세요.
...
18. ★표현 교정(광고주 피드백 반영) — 반드시 준수★:
  ...
  - ★"긍정적" 앞에 다른 글자가 붙은 어색한 표현(장긍정적·정긍정적·유긍정적·단긍정적·주긍정적 등)은 절대 쓰지 마세요. 원래 쓰려던 정확한 단어(적극적·정기적·유기적·장기적·주기적·꾸준한·지속적 등)를 쓰고, 애매하면 그냥 "긍정적"으로만 쓰세요. 예: "장긍정적인 관리" → "꾸준한 관리", "유긍정적으로 결합" → "유기적으로 결합".
  - 시술명은 반드시 "거우침"입니다. "거우치료"라고 줄여 쓰지 말고 "거우침 치료" 또는 "거우침"으로 쓰세요. 예: "한방 면역 거우치료" → "거우침 치료", "거우치료와 한약" → "거우침 치료와 한약".
...
- ★링크 금지★: 본문에 <a> 태그(하이퍼링크)를 절대 넣지 마세요. 다른 질환명, 외부 사이트, 내부 페이지 등 어떤 링크도 포함하지 마세요. 순수 텍스트와 구조 태그(<h2>, <h3>, <p>, <ul>, <ol>, <li>, <table>, <thead>, <tbody>, <tr>, <th>, <td>, <strong>, <em>)만 사용하세요.
- ★답변 포맷 매핑 — 제목 유형에 맞춰 본문 핵심부의 형식을 정하세요★
  * 정의·이유형(무엇인지·왜 그런지를 묻는 제목) → 핵심부를 <p> 문단 서술로 쓰세요. 억지로 목록·표를 만들지 마세요.
  * 방법·순서형(제목에 "방법", "하는 법", "순서", "단계"가 있을 때) → 핵심 절차를 <ol><li> 번호 리스트로 3~6단계로 정리하세요.
  * 비교·감별형(제목에 "비교", "차이", "vs", "구분"이 있을 때) → 비교가 글의 핵심일 때만 <table>로 정리하세요.
  * ★표는 필요할 때만★: 매 글에 억지로 넣지 마세요. 비교 대상이 명확히 2~3개이고 표로 볼 때 확실히 이해가 빠른 경우에만 사용합니다. 그 외에는 문단으로 서술하세요.
  * 표 크기 제한: 2~3열 × 5행 이내(모바일 가독성). 긴 문장 대신 짧은 구로 채우세요.
  * ★표 의료법 준수★: 표 안에 우열·순위·"더 효과적/더 좋다" 같은 평가 표현 금지. 타 치료법·타 병원 비방 금지. 오직 "방식의 차이"만 중립적으로 서술하세요.
  * 표 형식 예시: <table><thead><tr><th>구분</th><th>A 방식</th><th>B 방식</th></tr></thead><tbody><tr><td>접근 방법</td><td>…</td><td>…</td></tr></tbody></table>
  * 위 조건에 해당하지 않으면 기본은 문단 서술입니다.
...
const REVIEW_RULES = {
  ...
  medical: [
    ...
    { pattern: /기적적(인|으로)/g, replacement: '긍정적$1', category: '의료법', severity: 'high' },
  ...
  // 시술명 표기 교정 (거우침이 정식 명칭 — "거우치료" 축약 금지)
  terminology: [
    { pattern: /거우치료/g, replacement: '거우침 치료', category: '용어교정', severity: 'low' },
    // 과거 '기적' 치환 버그가 만든 garble을 원래 단어로 복원 (안전망)
    { pattern: /장긍정적/g, replacement: '장기적', category: '용어교정', severity: 'medium' },
    { pattern: /정긍정적/g, replacement: '정기적', category: '용어교정', severity: 'medium' },
    { pattern: /유긍정적/g, replacement: '유기적', category: '용어교정', severity: 'medium' },
    { pattern: /단긍정적/g, replacement: '단기적', category: '용어교정', severity: 'medium' },
    { pattern: /주긍정적/g, replacement: '주기적', category: '용어교정', severity: 'medium' },
  ],
};
