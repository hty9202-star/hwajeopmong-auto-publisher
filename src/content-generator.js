...
// 도입부 스타일 변형
const INTRO_STYLES = [
  '환자가 흔히 겪는 상황이나 고민을 서술형으로 시작하세요 (예: "거울을 볼 때마다 신경 쓰이는 잡티가 있거든요.")',
  '핵심 정의문으로 시작하세요 (예: "~은(는) ~에 의해 발생하는 피부 질환입니다")',
  '흥미로운 의학적 사실로 시작하세요 (예: "피부과를 찾는 환자 중 상당수가 이 질환을 갖고 있습니다.")',
  '계절이나 환경 변화에 연결하여 서술형으로 시작하세요 (예: "환절기가 되면 유독 심해지는 증상이 있습니다.")',
  '진료 현장 경험을 서술형으로 시작하세요 (예: "진료실에서 가장 많이 듣는 이야기 중 하나인데요.")',
];
...
1. 문단 길이: 한 문단은 2~3줄로 짧게 끊으세요. 4줄 이상 이어지는 긴 문단은 절대 금지입니다.
2. 문장 호흡: 한 문장은 짧게(15~30자 내외). 길게 늘어지는 복문보다 단문을 연결하는 스타일입니다.
3. 줄바꿈: 문장 사이에 줄바꿈을 적극적으로 사용하세요. 빽빽한 텍스트 블록을 피하세요.
4. 어조: 블로그에서 독자에게 편안하게 말하듯 서술형 대화체를 사용하세요. "~거든요", "~인데요", "~세요", "~드려요" 같은 부드러운 말투를 쓰세요. 단, 첫 문단에 "~하신 적 있으신가요?", "~고민이신가요?" 같은 억지 질문형은 금지합니다.
5. 도입부: 독자의 경험이나 공감할 수 있는 상황을 서술형으로 시작하세요. "~하신 적 있으신가요?" 같은 질문형 도입은 사용하지 마세요. 사전 정의식("~란 ~이다")도 금지입니다.
6. 구조: 번호(1, 2, 3...)로 섹션을 구분하는 네이버 블로그 스타일을 따르세요. H2/H3 소제목보다 번호 구분이 더 자연스럽습니다.
7. 전문성 표현: 의학 정보를 쉬운 비유와 일상 언어로 풀어 설명하세요. 교과서처럼 나열하지 마세요.
8. 브랜드 언급: 화접몽한의원은 글 중반~후반부에 치료 설명 흐름 속에서 2~3회만 자연스럽게 녹이세요.
9. 마무리: 부드럽게 내원을 권유하되 광고처럼 느껴지지 않게 작성하세요.
10. 전체 느낌: 전문 의료 블로그 글이지만, 친구에게 설명해주는 듯한 편안함이 있어야 합니다.
`;
  } catch (e) {
...
2. 소제목(H2) 스타일: 대화체·공감형·질문형으로 작성하세요. AI가 자연어 질문을 매칭할 수 있어야 합니다.
   - 좋은 예: "편평사마귀, 잡티인 줄 알았는데 자꾸 번져요", "왜 자꾸 재발하는 걸까요?", "한의원 치료는 어떻게 다를까요?"
...
[제목 작성 규칙 - 매우 중요]
- 40자 이내, 자연스러운 블로그 제목 스타일로 작성하세요
- 질환명(또는 지역+질환명)과 핵심 키워드를 포함하세요
- 괄호()를 사용하지 마세요
- 딱딱한 백과사전식 제목 금지: "~란? 원인·증상·치료법 완벽 가이드" 같은 패턴 절대 금지
- 블로그형 자연스러운 제목을 작성하세요. 아래 예시를 참고:
  * "편평사마귀 단순 제거만으로는 부족해요"
  * "여드름치료 흉터까지 잡는 한의원 관리법"
  * "모공각화증 치료가 필요한 순간"
  * "지루성피부염 화안케어로 관리하세요"
  * "모낭염 짜도 낫지 않는 뾰루지, 여드름부터 구별하세요"
  * "오돌토돌피부, 미세한 돌기가 느껴진다면"
  * "건선 내 피부가 보내는 오래된 신호"
- 이처럼 독자에게 말하듯 편안하고 구체적인 제목을 만드세요
...
` : ''}
[소제목(H2) 작성 규칙 - 매우 중요]
- 대화체·공감형·질문형으로 작성하세요. 딱딱한 백과사전식 소제목 금지!
- 좋은 예: "왜 자꾸 재발하는 걸까요?", "한의원 치료는 어떻게 다를까요?", "이런 증상이면 의심해보세요"
- 나쁜 예: "편평사마귀의 정의와 특징", "주요 원인", "치료 방법 및 예후"
- "~의 정의", "~의 원인", "~의 특징", "~의 개요" 패턴 절대 사용 금지
- 매번 다른 패턴(질문형, 공감형, 정보 요약형)을 혼용하세요
...
[작성 규칙 - 반드시 준수]
1. ★최우선★ 레퍼런스 톤 따르기: 위 레퍼런스 샘플과 동일한 글쓰기 스타일로 작성하세요. 레퍼런스의 문단 길이, 문장 호흡, 대화체 어조를 그대로 따라야 합니다. 이것이 가장 중요한 규칙입니다.
2. 문단 구조: 한 문단은 2~3줄로 짧게 끊으세요. 4줄 이상은 반드시 분리하세요. HTML에서 <p> 태그 하나에 2~3문장만 담으세요.
3. 절대 금지: 클릭베이트 표현 (충격, 놀라운, 폭발, 경악 등), 과장된 치료 효과 주장
4. 브랜드 언급: "${BRAND.name}" 브랜드명을 본문에서 2-3회만 자연스럽게 삽입 (과도한 반복 금지)
5. 문체: 편안한 서술형 대화체("~인데요", "~거든요", "~드려요", "~세요")를 기본으로 하되 전문성 유지. 첫 문단에 억지 질문("~하신 적 있으신가요?") 금지. 논문체·백과사전체 금지.
6. 구조: 전략(outline)에 정의된 구조를 따르되, 네이버 블로그처럼 읽기 쉬운 흐름으로 작성하세요.
7. 분량: 약 ${wordCount}자 내외
8. 전문 용어: 한의학 용어는 쉬운 비유와 함께 풀어서 설명 (교과서식 나열 금지)
9. 글 마무리: "개인별 체질과 증상에 따라 치료 결과가 다를 수 있습니다." 면책 문구 필수 포함
10. 다양성: 이전에 작성한 콘텐츠와 다른 표현, 비유, 예시를 사용하세요.
11. 토픽 집중: ${topic.name}에 대한 심도 있는 정보를 중심으로 작성하세요.
12. 언어: 모든 콘텐츠는 반드시 한국어로만 작성하세요. 영어 문장을 절대 포함하지 마세요. 의학 용어의 영문 병기(예: 아토피 피부염(Atopic Dermatitis))만 허용되며, 그 외 영어 표현은 일체 사용하지 마세요.
13. 출처: 학술 논문/학술지/연구 인용 절대 금지. 일반적인 의학 상식만 활용하세요.
${caseStudySafety}
...
다음 JSON 형식으로 작성된 콘텐츠를 출력하세요:
{
  ...
  "content": "작성된 HTML 본문 (레퍼런스 샘플과 동일한 톤·문단 길이로, <h2>, <p> 위주 사용, 한 <p>에 2~3문장만, 약 ${wordCount}자)",
  ${faqJsonField}
