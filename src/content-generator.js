...
const TREATMENT_MAP = {
  ...
  'keratosis-pilaris': { name: '화안케어', desc: '모공각화증은 화접몽한의원의 화안케어로 치료한다. 화안케어는 파우더필링 계열(Aoo 필링·해초필 계열의 정수)에 칵테일 기법을 적용한 외부 시술로, 각질의 두께·모공 속 염증 유무·착색 정도에 따라 약물 배합과 시술 강도를 맞춤 조절해 모공을 막은 각질을 정상화하고 피부결·착색을 개선한다. ※미세약초침이 아니라 파우더필링(칵테일 기법)임 — 여드름과 같은 파우더필링 계열. "미세약초침"이라는 표현을 쓰지 말 것. 모공각화증은 타입에 따라 치료 강도가 달라진다: (1) 매몰모형 — 각질과 모공 사이에 빠져나오지 못한 털이 피지·세균과 엉겨 검붉거나 갈색 착색을 만드는, 가장 흔하고 잘 치료되는 타입(여성 대부분)이며 각질 교체 주기를 정상화하면 빠르게 호전된다. (2) 호르몬민감형 — 모근이 호르몬에 민감해 모공 하나에서 털이 두 개 이상 나는 유형으로, 매몰모보다 붉고 깊으며 유전 영향이 커 Aoo 필링만으로는 한계가 있어 칵테일 기법 화안케어가 필요하다. (3) 홍반형 — 육안상 가장 심한 유형으로 90% 이상이 남성이고 군집 형태(사이사이 정상피부)를 띠어 칵테일 기법을 적용한 강력한 화안케어가 필요하다. 이태리타올로 미는 목욕, 손톱으로 뜯기, 면도가 대표적 악화 요인이다.' },
