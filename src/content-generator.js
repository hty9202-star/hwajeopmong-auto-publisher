...
- ★지역+브랜드 조합 표현 — 반드시 준수★: 제목에 지역(강남·강남역)과 브랜드(${BRAND.name})를 함께 넣을 때는 "강남에서 …"처럼 문두 부사어로 쓰지 말고, "강남 ${BRAND.name}"처럼 지역을 브랜드 바로 앞에 붙여 하나의 자연스러운 단위로 쓰세요. 예: (X) "강남에서 모낭염 재발을 멈추는 ${BRAND.name}의 치료법" → (O) "모낭염 재발을 멈추는 강남 ${BRAND.name} 치료법". 또한 지역·서술절·브랜드·치료법을 한 제목에 네 요소 모두 욱여넣어 늘어지지 않도록, 요소는 2~3개로 간결하게 유지하세요.
- ★이번 제목의 관점(이 관점을 우선 적용)★: ${(() => {
  ...
  var questionAngle = '질문형 — "' + t + '"에 대한 독자의 자연스러운 질문으로 시작하세요(예: "왜 자꾸…?", "…해도 되나요?", "…어떻게 해야 할까"). 억지 질문("있으신가요?")은 금지.';
  var angles = [
  ...
  // 질문형 확대 (나머지 중 약 40% → 전체의 약 32%)
  if (Math.random() < 0.4) return questionAngle;
