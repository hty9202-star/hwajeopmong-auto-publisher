...
const TREATMENT_MAP = {
  ...
  'acne-scars': { name: '리셀테라피(복합 흉터치료)', desc: '화접몽의 복합 여드름흉터 치료 프로그램인 리셀테라피로 진행한다. 과정은 한약도포 → 화안케어 → 고밀도 AMTS → 화안케어 순이다(과정을 나열할 때는 "한약도포"로 표기해도 된다). 다만 한약도포 단계의 세부 설명은 "불규칙한 형태로 자리잡은 흉터 표면을 일정 부분 제거하면서, 상처를 내는 약물로 흉터의 경계면을 자극한다"는 방식으로만 서술한다. ※세부 설명에서 한약도포를 "피부 재생을 돕는 한약 성분을 도포한다"는 식으로 풀지 말 것. 이를 통해 패인 흉터·색소침착을 개선한다. 리셀테라피는 연구논문으로 발표된 치료법이다.' },
