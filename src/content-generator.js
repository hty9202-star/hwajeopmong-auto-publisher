...
1. 문단 길이: 한 문단은 2~4줄로 짧게 끊으세요. 5줄 이상 이어지는 긴 문단은 절대 금지입니다.
2. 문장 호흡: 한 문장은 짧게(15~30자 내외). 길게 늘어지는 복문보다 단문을 연결하는 스타일입니다.
3. 줄바꿈: 문장 사이에 줄바꿈을 적극적으로 사용하세요. 빽빽한 텍스트 블록을 피하세요.
4. 어조: 블로그에서 독자에게 편안하게 말하듯 대화체를 사용하세요. "~있으신가요?", "~일까요?", "~인데요." 같은 자연스러운 말투를 섞으세요.
5. 도입부: 독자의 경험이나 공감할 수 있는 상황으로 시작하세요. 사전 정의식("~란 ~이다")으로 시작하지 마세요.
...
2. 문단 구조: 한 문단은 2~4줄로 짧게 끊으세요. 긴 문단은 분리하세요. HTML에서 <p> 태그 하나에 2~4문장만 담으세요.
3. 절대 금지: 클릭베이트 표현 (충격, 놀라운, 폭발, 경악 등), 과장된 치료 효과 주장
4. 브랜드 언급: "${BRAND.name}" 브랜드명을 본문에서 2-3회만 자연스럽게 삽입 (과도한 반복 금지)
5. 문체: 편안한 대화체("~인데요", "~있으신가요?", "~거든요")를 기본으로 하되 전문성을 유지. 딱딱한 논문체나 백과사전체 금지.
...
다음 JSON 형식으로 작성된 콘텐츠를 출력하세요:
{
  ...
  "content": "작성된 HTML 본문 (레퍼런스 샘플과 동일한 톤·문단 길이로, <h2>, <p> 위주 사용, 한 <p>에 2~4문장만, 약 ${wordCount}자)",
  ${faqJsonField}
